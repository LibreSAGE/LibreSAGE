	...
	if ((res=W3DShaderManager::getChipset()) != 0)
	{
		if (res >= DC_GENERIC_PIXEL_SHADER_1_1)
		{
			...
			D3DVERTEXELEMENT9 Declaration[]=
			{
				...
				{0, 16, D3DDECLTYPE_FLOAT2,   D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD, 0}, // Texture Coordinates
				D3DDECL_END()
			};

			//Monochrome pixel shader.
			hr = W3DShaderManager::LoadAndCreateD3DShader("shaders\\monochrome.pso", &Declaration[0], 0, false, (void**)&m_dwBWPixelShader);
			if (FAILED(hr))
				return FALSE;
...
Int ScreenCrossFadeFilter::set(enum FilterModes mode)
{
	...
	{	//rendering a quad with redirected rendering surface
		...
		{	//cross-fading using circle mask stored in stage 1
			...
			DX8Wrapper::Set_DX8_Texture_Sampler_State( 1, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP);
			DX8Wrapper::Set_DX8_Texture_Sampler_State( 1, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);
			DX8Wrapper::Set_DX8_Texture_Sampler_State( 1, D3DSAMP_MIPFILTER, D3DTEXF_NONE);
		}
...
Int TerrainShader2Stage::set(Int pass)																											  
{
	...
	if (TheGlobalData && TheGlobalData->m_bilinearTerrainTex || TheGlobalData->m_trilinearTerrainTex) {
		...
		DX8Wrapper::Set_DX8_Texture_Sampler_State(1, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(1, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
	} else {
		DX8Wrapper::Set_DX8_Texture_Sampler_State(0, D3DSAMP_MINFILTER, D3DTEXF_POINT);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(0, D3DSAMP_MAGFILTER, D3DTEXF_POINT);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(1, D3DSAMP_MINFILTER, D3DTEXF_POINT);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(1, D3DSAMP_MAGFILTER, D3DTEXF_POINT);
	}
	if (TheGlobalData && TheGlobalData->m_trilinearTerrainTex) {
		DX8Wrapper::Set_DX8_Texture_Sampler_State(0, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(1, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR);
	} else {
		DX8Wrapper::Set_DX8_Texture_Sampler_State(0, D3DSAMP_MIPFILTER, D3DTEXF_POINT);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(1, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR);
	}
	...
	switch (pass)
	{
		...
		case 2:
			...
			DX8Wrapper::Set_DX8_Texture_Stage_State(0,  D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_COUNT2);	
			DX8Wrapper::Set_DX8_Texture_Sampler_State(0,  D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP);
			DX8Wrapper::Set_DX8_Texture_Sampler_State(0,  D3DSAMP_ADDRESSV, D3DTADDRESS_WRAP);
			...
			if (W3DShaderManager::getCurrentShader() == W3DShaderManager::ST_TERRAIN_BASE_NOISE12)
			{
				...
				DX8Wrapper::Set_DX8_Texture_Sampler_State( 1, D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP);
				DX8Wrapper::Set_DX8_Texture_Sampler_State( 1, D3DSAMP_ADDRESSV, D3DTADDRESS_WRAP);
	...
	if (terrainShader2Stage.init() && (res=W3DShaderManager::getChipset()) >= DC_GENERIC_PIXEL_SHADER_1_1)
	{
		if (res >= DC_GENERIC_PIXEL_SHADER_1_1)
		{
			...
			D3DVERTEXELEMENT9 Declaration[]=
			{
				...
				{0, 12, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_COLOR,    0}, // Diffuse
				{0, 16, D3DDECLTYPE_FLOAT2,   D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD, 0}, // Texture Coordinates
				{0, 24, D3DDECLTYPE_FLOAT2,   D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD, 1}, // Texture Coordinates 2
				D3DDECL_END()
			};

			//base version which doesn't apply any noise textures.
			HRESULT hr = W3DShaderManager::LoadAndCreateD3DShader("shaders\\terrain.pso", &Declaration[0], 0, false, (void**)&m_dwBasePixelShader);
			if (FAILED(hr))
				return FALSE;

			//version which blends 1 noise texture.
			hr = W3DShaderManager::LoadAndCreateD3DShader("shaders\\terrainnoise.pso", &Declaration[0], 0, false, (void**)&m_dwBaseNoise1PixelShader);
			if (FAILED(hr))
				return FALSE;

			//version which blends 2 noise textures.
			hr = W3DShaderManager::LoadAndCreateD3DShader("shaders\\terrainnoise2.pso", &Declaration[0], 0, false, (void**)&m_dwBaseNoise2PixelShader);
			if (FAILED(hr))
				return FALSE;
...
{	
	...
	{	
		...
		DX8Wrapper::Set_DX8_Texture_Sampler_State(2,  D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(2,  D3DSAMP_ADDRESSV, D3DTADDRESS_WRAP);
		
		if (W3DShaderManager::getCurrentShader() == W3DShaderManager::ST_TERRAIN_BASE_NOISE12)
		{	//full shader
			DX8Wrapper::Set_DX8_Texture_Sampler_State(3,  D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP);
			DX8Wrapper::Set_DX8_Texture_Sampler_State(3,  D3DSAMP_ADDRESSV, D3DTADDRESS_WRAP);
			...
			DX8Wrapper::Set_DX8_Texture_Sampler_State(2, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
			DX8Wrapper::Set_DX8_Texture_Sampler_State(2, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);

			DX8Wrapper::Set_DX8_Texture_Sampler_State(3, D3DSAMP_MINFILTER, D3DTEXF_POINT);
			DX8Wrapper::Set_DX8_Texture_Sampler_State(3, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
		...
		{	//single noise texture shader
			...
			{	//light map
				...
				terrainShader2Stage.updateNoise2(((D3DXMATRIX*)&curView),&inv);	//update curView with texture matrix
				DX8Wrapper::Set_DX8_Texture_Sampler_State(2, D3DSAMP_MINFILTER, D3DTEXF_POINT);
				DX8Wrapper::Set_DX8_Texture_Sampler_State(2, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
			}
			DX8Wrapper::_Set_DX8_Transform(D3DTS_TEXTURE2, curView);
		}
...
Int CloudTextureShader::set(Int stage)
{
	...
	DX8Wrapper::Set_DX8_Texture_Sampler_State(stage, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
	DX8Wrapper::Set_DX8_Texture_Sampler_State(stage, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
...
Int RoadShaderPixelShader::set(Int pass)
{
	...
	{	DX8Wrapper::Set_DX8_Texture_Sampler_State(0, D3DSAMP_MIPFILTER, D3DTEXF_POINT);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(1, D3DSAMP_MIPFILTER, D3DTEXF_POINT);
	}
...
{	
	...
	HRESULT hr = DX8Wrapper::_Get_D3D_Device8()->SetRenderTarget(0,m_newRenderSurface); DX8Wrapper::_Get_D3D_Device8()->SetDepthStencilSurface(m_oldDepthSurface);
...
{	
	...
	HRESULT hr = DX8Wrapper::_Get_D3D_Device8()->SetRenderTarget(0,m_oldRenderSurface); DX8Wrapper::_Get_D3D_Device8()->SetDepthStencilSurface(m_oldDepthSurface);	//restore original render target
	DEBUG_ASSERTCRASH(hr==S_OK, ("Set target failed unexpectedly."));
	if (hr == S_OK)
	{
...
ChipsetType W3DShaderManager::getChipset( void )
{
	...
	/*	HRESULT res = */ d3d8Interface->GetAdapterIdentifier(0,0,&did);
...
//=============================================================================
HRESULT W3DShaderManager::LoadAndCreateD3DShader(const char* strFilePath, const D3DVERTEXELEMENT9* pDeclaration, DWORD Usage, Bool ShaderType, void** pHandle)
{
	...
	try
	{
		...
		// D3D9: vertex declarations are separate objects (created by the caller via
		// CreateVertexDeclaration from pDeclaration); shader creation no longer takes
		// a declaration or a usage flag.
		(void)pDeclaration;
		(void)Usage;
...
Bool testMinimumRequirements(ChipsetType *videoChipType, CpuType *cpuType, Int *cpuFreq, Int64 *numRAM, Real *intBenchIndex, Real *floatBenchIndex, Real *memBenchIndex)
{
...
Bool W3DShaderManager::testMinimumRequirements(ChipsetType *videoChipType, CpuType *cpuType, Int *cpuFreq, Int64 *numRAM, Real *intBenchIndex, Real *floatBenchIndex, Real *memBenchIndex)
{
	if (videoChipType)
...
Int FlatTerrainShaderPixelShader::set(Int pass)
{
	...
	DX8Wrapper::Set_DX8_Texture_Sampler_State( curStage, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP);
	DX8Wrapper::Set_DX8_Texture_Sampler_State( curStage, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);
	//tell pixel shader which UV set to use for each stage
	...
	if (TheGlobalData && TheGlobalData->m_bilinearTerrainTex || TheGlobalData->m_trilinearTerrainTex) {
		DX8Wrapper::Set_DX8_Texture_Sampler_State(curStage, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(curStage, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
	} else {
		DX8Wrapper::Set_DX8_Texture_Sampler_State(curStage, D3DSAMP_MINFILTER, D3DTEXF_POINT);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(curStage, D3DSAMP_MAGFILTER, D3DTEXF_POINT);
	}
	if (TheGlobalData && TheGlobalData->m_trilinearTerrainTex) {
		DX8Wrapper::Set_DX8_Texture_Sampler_State(curStage, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR);
	} else {
		DX8Wrapper::Set_DX8_Texture_Sampler_State(curStage, D3DSAMP_MIPFILTER, D3DTEXF_POINT);
	}
	...
	if (doNoise1) {	 // Cloud pass.
		...
		DX8Wrapper::Set_DX8_Texture_Sampler_State(curStage,  D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(curStage,  D3DSAMP_ADDRESSV, D3DTADDRESS_WRAP);
