...
void W3DTreeBuffer::freeTreeBuffers(void)
{
	...
	if (m_pTreeVertexDeclaration)
		m_pTreeVertexDeclaration->Release();
	m_pTreeVertexDeclaration = NULL;
...
void W3DTreeBuffer::allocateTreeBuffers(void)
{
	...
	D3DVERTEXELEMENT9 Declaration[] =
	{
		...
		{0, 24, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_COLOR,    0}, // Diffuse color
		{0, 28, D3DDECLTYPE_FLOAT2,   D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD, 0}, // Tex coord
		D3DDECL_END()
	};

	HRESULT hr = W3DShaderManager::LoadAndCreateD3DShader("shaders\\Trees.vso", &Declaration[0], 0, true, (void**)&m_dwTreeVertexShader);

	// D3D9: the vertex declaration is a standalone object, no longer fused into the
	// vertex shader. Create it here so it can be bound alongside the vertex shader.
	if (SUCCEEDED(hr))
		hr = DX8Wrapper::_Get_D3D_Device8()->CreateVertexDeclaration(Declaration, &m_pTreeVertexDeclaration);

	if (SUCCEEDED(hr))
		hr = W3DShaderManager::LoadAndCreateD3DShader("shaders\\Trees.pso", &Declaration[0], 0, false, (void**)&m_dwTreePixelShader);

	if (FAILED(hr)) {
		// Release whatever was created so we neither leak COM objects nor leave the
		// buffer half-initialised: drawTrees() keys off m_dwTreeVertexShader and would
		// otherwise bind a null vertex declaration.
		if (m_dwTreePixelShader) {
			m_dwTreePixelShader->Release();
			m_dwTreePixelShader = NULL;
		}
...
//#define DEBUG_TEXTURE 1
	...
	if (m_dwTreeVertexShader) {
		...
		DX8Wrapper::_Get_D3D_Device8()->SetVertexDeclaration(m_pTreeVertexDeclaration);
