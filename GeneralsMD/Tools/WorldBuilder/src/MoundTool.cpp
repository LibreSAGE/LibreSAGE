...
// MoundTool.cpp
// Height mound/dig tool for worldbuilder.
...
MoundTool::MoundTool(void) :
	Tool(ID_MOUND_TOOL, ":/cursors/cross.cur")
{
...
void MoundTool::mouseDown(TTrackingMode m, QPoint viewPt, WbView* pView, CWorldBuilderDoc *pDoc)
{
	...
	m_lastMoveTime = moundTickCount();
	m_lastMoveTime -= MIN_DELAY_TIME + 1; // Make the tool fire the first time.
	mouseMoved(m, viewPt, pView, pDoc);
}

void MoundTool::mouseUp(TTrackingMode m, QPoint viewPt, WbView* pView, CWorldBuilderDoc *pDoc)
{
	if (m != TRACK_L) return;
...
void MoundTool::mouseMoved(TTrackingMode m, QPoint viewPt, WbView* pView, CWorldBuilderDoc *pDoc)
{
	Coord3D cpt;
