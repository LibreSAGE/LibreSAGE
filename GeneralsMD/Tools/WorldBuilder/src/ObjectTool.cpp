...
// ObjectTool.cpp
// Object placement tool for worldbuilder (Qt6 port).
...
ObjectTool::ObjectTool(void) :
	Tool(ID_OBJECT_TOOL, ":/cursors/IDC_POINTER.cur")
{
}
...
void ObjectTool::deactivate()
{
	...
	if (p3View) p3View->setObjTracking(NULL, m_downPt3d, 0, false);
}

/// Shows the object options panel
void ObjectTool::activate()
{
	CMainFrame::GetMainFrame()->showOptionsDialog(ID_OBJECT_TOOL);
...
/** Execute the tool on mouse down - remember the anchor point. */
void ObjectTool::mouseDown(TTrackingMode m, QPoint viewPt, WbView* pView, CWorldBuilderDoc *pDoc)
{
	if (m != TRACK_L) return;
...
/** Tracking - show the transparent preview of the object. */
void ObjectTool::mouseMoved(TTrackingMode m, QPoint viewPt, WbView* pView, CWorldBuilderDoc *pDoc)
{
	...
	if (m == TRACK_L) {	// Mouse is down, so preview the angle if > hysteresis.
		...
		justAClick = (abs(viewPt.x() - m_downPt2d.x())<HYSTERESIS && abs(viewPt.y() - m_downPt2d.y())<HYSTERESIS);
		loc = m_downPt3d;
	}
...
/** Execute the tool on mouse up - place the object. */
void ObjectTool::mouseUp(TTrackingMode m, QPoint viewPt, WbView* pView, CWorldBuilderDoc *pDoc)
{
	if (m != TRACK_L) return;

	// always check hysteresis in view coords.
	Bool justAClick = (abs(viewPt.x() - m_downPt2d.x())<HYSTERESIS && abs(viewPt.y() - m_downPt2d.y())<HYSTERESIS);
