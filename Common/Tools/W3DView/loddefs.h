...
//	Inline functions
//
inline bool Is_LOD_Name_Valid (const char* name, LOD_NAMING_TYPE &type)
{
	bool retval = false;

	// Does this name fit with the format expected?
	const char* last_2_chars = name + (::lstrlen (name)-2);
...
inline bool Is_Model_Part_of_LOD (const char* name, const char* base, LOD_NAMING_TYPE &type)
{
	// Assume failure
	bool retval = false;
	...
	if (::strstr (name, base) == name) {
		...
		const char* extension = name + ::lstrlen (base);
