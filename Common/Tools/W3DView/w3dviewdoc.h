...
public:
	...
	HLodPrototypeClass *GenerateLOD (const char* pszLODBaseName, LOD_NAMING_TYPE type);
	...
	BOOL					SaveSettings (const char* pszFilename, DWORD dwSettingsMask);
	BOOL					LoadSettings (const char* pszFileName);
	GraphicView *		GetGraphicView (void);
	CDataTreeView *	GetDataTreeView (void);

	void					Build_Emitter_List (EmitterInstanceListClass *emitter_list, const char* emitter_name, RenderObjClass *render_obj = NULL);
	...
	void					PlayAnimation (RenderObjClass *pobj, const char* panim_name = NULL, bool use_global_reset_flag = true, bool allow_reset = true);
	...
	//
	const QString&	GetBackgroundBMP  (void) const						{ return m_stringBackgroundBMP; }
	void					SetBackgroundBMP  (const char* pszBackgroundBMP);

	//
	//  Background Object methods
	//
	const QString&	GetBackgroundObjectName (void) const				{ return m_stringBackgroundObject; }
	...
	bool					Save_Current_Emitter (const QString &filename);

	//
	//	Primitive serialization methods
	//
	bool					Save_Selected_Primitive (void);
	bool					Save_Current_Sphere (const QString &filename);
	bool					Save_Current_Ring (const QString &filename);
	...
	bool					Save_Current_Aggregate (const QString &filename);		  

	//
	//	Sound object methods
	//
	bool					Save_Selected_Sound_Object (void);
	bool					Save_Current_Sound_Object (const QString &filename);

	//
	//  LOD methods
	//
	bool					Save_Current_LOD (const QString &filename);
	...
	bool					Lookup_Path (const char* asset_name, QString &path);
	const QString &		Get_Last_Path (void) const { return m_LastPath; }

	//
	//	Texture search paths
	//
	const QString &	Get_Texture_Path1 (void) const { return m_TexturePath1; }
	const QString &	Get_Texture_Path2 (void) const { return m_TexturePath2; }
...
private:
	...
	QString					m_TexturePath1;
	QString					m_TexturePath2;

	QString					m_LastPath;

	DynamicVectorClass<QString>	m_LoadList;
};