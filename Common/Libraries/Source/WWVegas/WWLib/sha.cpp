 ...
 *   07/03/1996 JLB : Created.                                                                 *
 *=============================================================================================*/
void SHAEngine::Hash(void const * data, uint32_t length)
{
	IsCached = false;
	...
	*/
	...
	for (int32_t bcount = 0; bcount < blocks; bcount++) {
...
int SHAEngine::Result(void * result) const
{
	...
	*/
	...
	int32_t partialcount = PartialCount;
	...
	*(int32_t *)(&partial[SRC_BLOCK_SIZE-4]) = Reverse_LONG((length*8));
	Process_Block(&partial[0], acc);

	memcpy((char *)&FinalResult, &acc, sizeof(acc));
	for (int32_t index = 0; index < sizeof(FinalResult)/sizeof(int32_t); index++) {
//	for (int index = 0; index < SRC_BLOCK_SIZE/sizeof(long); index++) {
		(int32_t &)FinalResult.Long[index] = Reverse_LONG(FinalResult.Long[index]);
	}
	(bool&)IsCached = true;
	memcpy(result, &FinalResult, sizeof(FinalResult));
...
inline int32_t _rotl(int32_t X, int n)
{
	return(int32_t)( ( X << n ) | ( (uint32_t)X >> ((sizeof(int32_t)*8) - n) ) );
}

inline uint32_t _rotl(uint32_t X, int n)
{
	return(uint32_t)( ( X << n ) | ( (uint32_t)X >> ((sizeof(uint32_t)*8) - n) ) );
}
