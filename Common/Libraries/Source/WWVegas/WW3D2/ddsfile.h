...
struct LegacyDDSURFACEDESC2 {
	...
	unsigned AlphaBitDepth;
	unsigned Reserved;
	uint32_t Surface;	// 32-bit field; original DX7 layout used a 32-bit lpSurface pointer slot, keep it 32-bit so the on-disk struct size stays 124 bytes on 64-bit platforms
	union
	{
