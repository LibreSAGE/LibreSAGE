...
int								DX8Wrapper::BaseVertexIndex = 0;
...
void DX8Wrapper::Set_Default_Global_Render_States(void)
{
	...
	// [D3D9 removed - ZBias now via projection matrix] Set_DX8_Render_State(D3DRS_ZBIAS,0);
...
bool DX8Wrapper::Create_Device(void)
{
	...
	(
		...
		( 
			...
			(
				CurRenderDevice,
				0/*D3DENUM_WHQL_LEVEL removed in D3D9*/,
...
void DX8Wrapper::Enumerate_Devices()
{
	...
	for (int adapter_index=0; adapter_index<adapter_count; adapter_index++) {
		...
		HRESULT res = D3DInterface->GetAdapterIdentifier(adapter_index,0/*D3DENUM_WHQL_LEVEL removed in D3D9*/,&id);
		...
		if (res == D3D_OK) {
			...
			*/
			...
			D3DInterface->GetAdapterIdentifier(adapter_index,0/*D3DENUM_WHQL_LEVEL removed in D3D9*/,&desc.AdapterIdentifier);
	...
	unsigned short vertex_count)
{
	...
	// If using FVF format VB, set the fixed-function vertex format (D3D9 SetFVF)
	...
	// D3D9 moved the base vertex index off SetIndices onto DrawIndexedPrimitive.
	DX8CALL(SetIndices(
...
//#ifndef _XBOX
	...
	if (render_target == NULL || render_target == DefaultRenderTarget) 
	{
		...
		{
			DX8CALL(SetRenderTarget (0, DefaultRenderTarget)); DX8CALL(SetDepthStencilSurface (DefaultDepthBuffer));
...
//		IDirect3DSurface9 *depth_buffer = NULL;
		...
		if (CurrentRenderTarget != NULL) 
		{
			...
			{
				DX8CALL(SetRenderTarget (0, CurrentRenderTarget)); DX8CALL(SetDepthStencilSurface (DefaultDepthBuffer));
			}
			else 
			{
				DX8CALL(SetRenderTarget (0, CurrentRenderTarget)); DX8CALL(SetDepthStencilSurface (NULL));
			}
		}
	}
...
//		IDirect3DSurface9 *depth_buffer = NULL;
		...
		if (CurrentRenderTarget != NULL) 
		{
			...
			//	Switch render targets
			//
			DX8CALL(SetRenderTarget (0, CurrentRenderTarget)); DX8CALL(SetDepthStencilSurface (CurrentDepthBuffer));
		}
	}
...
// D3D9 has no CopyRects. UpdateSurface handles the fast SYSTEMMEM->DEFAULT path,
// but not MANAGED destinations (e.g. the font glyph cache textures) -- for those we
// fall back to a format-aware lock/copy via D3DXLoadSurfaceFromSurface. All engine
// callers copy between lockable surfaces, so this covers every case; render-target
// read-back paths use GetRenderTargetData directly instead.
	...
	CONST POINT* pDestPointsArray)
{
	DX8_Assert();
	if (cRects == 0) {
		if (FAILED(D3DDevice->UpdateSurface(pSourceSurface, NULL, pDestinationSurface, NULL))) {
			D3DXLoadSurfaceFromSurface(pDestinationSurface, NULL, NULL, pSourceSurface, NULL, NULL, D3DX_FILTER_NONE, 0);
		}
	} else {
		for (UINT i = 0; i < cRects; i++) {
			const RECT& srcRect = pSourceRectsArray[i];
			POINT dstPoint = pDestPointsArray ? pDestPointsArray[i] : POINT{ srcRect.left, srcRect.top };
			if (FAILED(D3DDevice->UpdateSurface(pSourceSurface, &srcRect, pDestinationSurface, &dstPoint))) {
				RECT dstRect = { dstPoint.x, dstPoint.y,
					dstPoint.x + (srcRect.right - srcRect.left),
					dstPoint.y + (srcRect.bottom - srcRect.top) };
				D3DXLoadSurfaceFromSurface(pDestinationSurface, NULL, &dstRect, pSourceSurface, NULL, &srcRect, D3DX_FILTER_NONE, 0);
			}
		}
	}
}
...
{
	DX8_Assert();
	// D3D9 removed ResourceManagerDiscardBytes; EvictManagedResources evicts all
	// managed resources (there is no byte-count granularity).
	(void)bytes;
...
/*	Set_DX8_Render_State(D3DRS_LOCALVIEWER, TRUE);
	...
	// [D3D9 removed - use SetSoftwareVertexProcessing] Set_DX8_Render_State(D3DRS_SOFTWAREVERTEXPROCESSING, FALSE);
	...
	for (i=0; i<CurrentCaps->Get_Max_Textures_Per_Pass(); i++)
	{
		...
		Set_DX8_Texture_Sampler_State(i, D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP);
		Set_DX8_Texture_Sampler_State(i, D3DSAMP_ADDRESSV, D3DTADDRESS_WRAP);
		Set_DX8_Texture_Sampler_State(i, D3DSAMP_BORDERCOLOR, 0);
//		Set_DX8_Texture_Sampler_State(i, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
//		Set_DX8_Texture_Sampler_State(i, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
//		Set_DX8_Texture_Sampler_State(i, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR);
//		Set_DX8_Texture_Sampler_State(i, D3DSAMP_MIPMAPLODBIAS, 0);
//		Set_DX8_Texture_Sampler_State(i, D3DSAMP_MAXMIPLEVEL, 0);
//		Set_DX8_Texture_Sampler_State(i, D3DSAMP_MAXANISOTROPY, 1);
		//Set_DX8_Texture_Sampler_State(i, D3DSAMP_ADDRESSW, D3DTADDRESS_WRAP);
...
const char* DX8Wrapper::Get_DX8_Render_State_Name(D3DRENDERSTATETYPE state)
{
	...
	// [D3D9 removed] case D3DRS_LINEPATTERN                   : return "D3DRS_LINEPATTERN";
	...
	// [D3D9 removed] case D3DRS_ZVISIBLE                      : return "D3DRS_ZVISIBLE";
	...
	// [D3D9 removed] case D3DRS_EDGEANTIALIAS                 : return "D3DRS_EDGEANTIALIAS";
	// [D3D9 removed] case D3DRS_ZBIAS                         : return "D3DRS_ZBIAS";
	...
	// [D3D9 removed] case D3DRS_SOFTWAREVERTEXPROCESSING      : return "D3DRS_SOFTWAREVERTEXPROCESSING";
	...
	// [D3D9 removed] case D3DRS_PATCHSEGMENTS                 : return "D3DRS_PATCHSEGMENTS";
...
const char* DX8Wrapper::Get_DX8_Texture_Stage_State_Name(D3DTEXTURESTAGESTATETYPE state)
{
	...
	// [D3D9 removed] case D3DTSS_ADDRESSU                  : return "D3DTSS_ADDRESSU";
	// [D3D9 removed] case D3DTSS_ADDRESSV                  : return "D3DTSS_ADDRESSV";
	// [D3D9 removed] case D3DTSS_BORDERCOLOR               : return "D3DTSS_BORDERCOLOR";
	// [D3D9 removed] case D3DTSS_MAGFILTER                 : return "D3DTSS_MAGFILTER";
	// [D3D9 removed] case D3DTSS_MINFILTER                 : return "D3DTSS_MINFILTER";
	// [D3D9 removed] case D3DTSS_MIPFILTER                 : return "D3DTSS_MIPFILTER";
	// [D3D9 removed] case D3DTSS_MIPMAPLODBIAS             : return "D3DTSS_MIPMAPLODBIAS";
	// [D3D9 removed] case D3DTSS_MAXMIPLEVEL               : return "D3DTSS_MAXMIPLEVEL";
	// [D3D9 removed] case D3DTSS_MAXANISOTROPY             : return "D3DTSS_MAXANISOTROPY";
	...
	// [D3D9 removed] case D3DTSS_ADDRESSW                  : return "D3DTSS_ADDRESSW";
...
void DX8Wrapper::Get_DX8_Render_State_Value_Name(StringClass& name, D3DRENDERSTATETYPE state, unsigned value)
{
	...
	// [D3D9 removed] case D3DRS_LINEPATTERN:
	case D3DRS_FOGCOLOR:
	case D3DRS_ALPHAREF:
	...
	// [D3D9 removed] case D3DRS_EDGEANTIALIAS:
	case D3DRS_CLIPPING:
	case D3DRS_LIGHTING:
	case D3DRS_COLORVERTEX:
	case D3DRS_LOCALVIEWER:
	case D3DRS_NORMALIZENORMALS:
	// [D3D9 removed] case D3DRS_SOFTWAREVERTEXPROCESSING:
	...
	// [D3D9 removed] case D3DRS_ZVISIBLE:
		name="NOTSUPPORTED";
		break;
	...
	// [D3D9 removed] case D3DRS_PATCHSEGMENTS:
	case D3DRS_POINTSIZE_MAX:
	case D3DRS_TWEENFACTOR:
		name.Format("%f",*(float*)&value);
		break;

	// [D3D9 removed] case D3DRS_ZBIAS:
...
void DX8Wrapper::Get_DX8_Texture_Stage_State_Value_Name(StringClass& name, D3DTEXTURESTAGESTATETYPE state, unsigned value)
{
	...
	// [D3D9 removed] case D3DTSS_ADDRESSU:
	// [D3D9 removed] case D3DTSS_ADDRESSV:
	// [D3D9 removed] case D3DTSS_ADDRESSW:
		name=Get_DX8_Texture_Address_Name(value);
		break;

	// [D3D9 removed] case D3DTSS_MAGFILTER:
	// [D3D9 removed] case D3DTSS_MINFILTER:
	// [D3D9 removed] case D3DTSS_MIPFILTER:
	...
	// [D3D9 removed] case D3DTSS_MIPMAPLODBIAS:
	...
	// Integer value
	// [D3D9 removed] case D3DTSS_MAXMIPLEVEL:
	// [D3D9 removed] case D3DTSS_MAXANISOTROPY:
		name.Format("%d",value);
		break;
	// Hex values
	// [D3D9 removed] case D3DTSS_BORDERCOLOR:
		name.Format("0x%x",value);
		break;
