...
// DirectDraw is not available on non-Windows platforms. The DDS surface
// description is parsed via the platform-independent LegacyDDSURFACEDESC2
// structures declared in ddsfile.h, so the only thing actually needed from
// <ddraw.h> are the following capability flags. Define them here with their
// standard DirectDraw values so the file builds cross-platform.
