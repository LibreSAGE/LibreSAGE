...
)
{
	...
	if ((buffer != NULL) &&
		 (string_id != NULL) &&
		 (buffer->Get_Raw_Length () < (uint32_t)(m_MaxCacheSize / 2))) {
...
WWAudioClass::Get_2D_Sample (const AudibleSoundClass &sound_obj)
{
	...
	for (int index = 0; (index < m_2DSampleHandles.Count ()) && !found; index ++) {
		...
		if (sample != NULL) {
			...
			AudibleSoundClass *sound_obj = NULL;
#ifdef SAGE_USE_MILES
			sound_obj = (AudibleSoundClass *)::AIL_sample_user_data (sample, INFO_OBJECT_PTR);
#endif
			if (sound_obj == NULL) {
...
WWAudioClass::Get_3D_Sample (const Sound3DClass &sound_obj)
{
	...
	for (int index = 0; (index < m_3DSampleHandles.Count ()) && !found; index ++) {
		...
		if (sample != NULL) {
			...
			AudibleSoundClass *sound_obj = NULL;
#ifdef SAGE_USE_MILES
			sound_obj = (AudibleSoundClass *)::AIL_3D_object_user_data (sample, INFO_OBJECT_PTR);
#endif
			if (sound_obj == NULL) {
