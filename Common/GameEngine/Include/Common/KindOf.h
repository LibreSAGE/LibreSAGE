...
enum KindOfType : int {
  ...
  KINDOF_EMP_HARDENED,            ///< Like a delivery plane (B52, B3, CargoPlane,etc.)  or a SpectreGunship, which sort-of IS the weapon...
	...
	// Generals-only. Zero Hour replaced this with KINDOF_FS_AIRFIELD, but Generals' code and its
	// retail INI data still use the "AIRFIELD" name, so it lives on at the end of the list --
	// appending keeps every Zero Hour bit index unchanged. Inert for Zero Hour.
...
struct KindOfMaskTag;
typedef BitFlags<KINDOF_COUNT, KindOfMaskTag>	KindOfMaskType;
