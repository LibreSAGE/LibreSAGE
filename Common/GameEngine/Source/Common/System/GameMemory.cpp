...
#define DECLARE_PERF_TIMER(x)	// do nothing, for now
#define USE_PERF_TIMER(x)		// do nothing, for now
...
#ifdef MEMORYPOOL_SINGLEBLOCK_GETS_STACKTRACE
	if (theStackTraceDepth > 0 && TheGameMemoryCheckForLeaks) 
	{
...
#endif
{
	...
	#ifdef USE_FILLER_VALUE
	calcFillerValue(SDL_rand(MAX_INIT_FILLER_COUNT-1));
	#endif
#endif
}
//-----------------------------------------------------------------------------
...
void MemoryPoolFactory::debugSetInitFillerIndex(Int index)
{
	...
	if (index < 0 || index >= MAX_INIT_FILLER_COUNT)
		index = SDL_rand(MAX_INIT_FILLER_COUNT-1);
...
void MemoryPoolFactory::memoryPoolUsageReport( const char* filename, FILE *appendToFileInstead )
{
	...
	if( !appendToFileInstead )
	{
		...
		tmp[sizeof(tmp) - 1] = '\0';
		strncat(tmp,".csv",sizeof(tmp) - strlen(tmp) - 1);
...
void MemoryPoolFactory::debugMemoryReport(Int flags, Int startCheckpoint, Int endCheckpoint, FILE *fp )
{
	...
	if (doBlockReport)
	{
		...
		DEBUG_LOG(("------------------------------------------\n"));
		char buf[256] = "";
		if (flags & _REPORT_CP_ALLOCATED_BEFORE) strncat(buf, "AllocBefore ", sizeof(buf) - strlen(buf) - 1);
		if (flags & _REPORT_CP_ALLOCATED_BETWEEN) strncat(buf, "AllocBetween ", sizeof(buf) - strlen(buf) - 1);
		if (flags & _REPORT_CP_FREED_BEFORE) strncat(buf, "FreedBefore ", sizeof(buf) - strlen(buf) - 1);
		if (flags & _REPORT_CP_FREED_BETWEEN) strncat(buf, "FreedBetween ", sizeof(buf) - strlen(buf) - 1);
		if (flags & _REPORT_CP_FREED_NEVER) strncat(buf, "StillExisting ", sizeof(buf) - strlen(buf) - 1);
