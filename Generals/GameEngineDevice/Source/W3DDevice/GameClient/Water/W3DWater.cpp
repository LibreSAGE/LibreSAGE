...
void WaterRenderObjClass::ReleaseResources(void)
{
	...
	if (m_pWaveVertexDeclaration)
		m_pWaveVertexDeclaration->Release();
...
void WaterRenderObjClass::ReAcquireResources(void)
{
	...
	{	//pixel/vertex shader based water assets.
		...
		hr = W3DShaderManager::LoadAndCreateD3DShader("shaders\\wave.pso", &Declaration[0], 0, false, (void**)&m_dwWavePixelShader);
		if (FAILED(hr))
			return;

		hr = W3DShaderManager::LoadAndCreateD3DShader("shaders\\wave.vso", &Declaration[0], 0, true, (void**)&m_dwWaveVertexShader);
		if (FAILED(hr))
			return;

		// D3D9: create the standalone vertex declaration for the wave vertex shader.
		hr = DX8Wrapper::_Get_D3D_Device8()->CreateVertexDeclaration(Declaration, &m_pWaveVertexDeclaration);
		if (FAILED(hr))
			return;
...
#else
		#pragma message("Need to create shaders for non windows platform")
#endif
	}
...
//	m_pDev->SetSamplerState( 1, D3DSAMP_MIPFILTER, D3DTEXF_NONE );
	...
	m_pDev->SetVertexDeclaration(m_pWaveVertexDeclaration);
...
//	m_pDev->SetRenderState(D3DRS_FILLMODE,D3DFILL_SOLID);
	...
	m_pDev->SetVertexShader(NULL);	//D3D9: clear programmable vertex shader (SetFVF no longer does)
	...
	{	if (TheTerrainRenderObject->getShroud())
		{
			...
			m_pDev->SetSamplerState( 3, D3DSAMP_MINFILTER, D3DTEXF_LINEAR );
			m_pDev->SetSamplerState( 3, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR );
			m_pDev->SetSamplerState( 3, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP);
			m_pDev->SetSamplerState( 3, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);
			//Shroud shader uses z-compare of EQUAL which wouldn't work on water because it doesn't
			//write to the zbuffer.  Change to LESSEQUAL.
			DX8Wrapper::_Get_D3D_Device8()->SetRenderState(D3DRS_ZFUNC, D3DCMP_LESSEQUAL);
		}
