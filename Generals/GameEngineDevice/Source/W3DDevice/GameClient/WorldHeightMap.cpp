...
#endif
	m_numCliffInfo(1),
	m_terrainTex(NULL), m_alphaTerrainTex(NULL), m_alphaEdgeTex(NULL), m_alphaEdgeHeight(1), m_numBitmapTiles(0), m_numBlendedTiles(1)
{
	Int i;
	for (i=0; i<NUM_SOURCE_TILES; i++) {
...
TextureClass *WorldHeightMap::getTerrainTexture(void)
{
	...
	if (m_terrainTex == NULL) {
		...
		char buf[64];
		snprintf(buf, sizeof(buf), "Base tex height %d\n", pow2Height);
		DEBUG_LOG(("%s", buf));
		REF_PTR_RELEASE(m_alphaTerrainTex);
