...
void TerrainTextureClass::Apply(unsigned int stage)
{
	...
	if (TheGlobalData && TheGlobalData->m_bilinearTerrainTex || TheGlobalData->m_trilinearTerrainTex) {
		...
		DX8Wrapper::Set_DX8_Texture_Sampler_State(stage, D3DSAMP_MINFILTER, D3DTEXF_POINT);
		DX8Wrapper::Set_DX8_Texture_Sampler_State(stage, D3DSAMP_MAGFILTER, D3DTEXF_POINT);
	}
	if (TheGlobalData && TheGlobalData->m_trilinearTerrainTex) {
		DX8Wrapper::Set_DX8_Texture_Sampler_State(stage, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR);
	} else {
		DX8Wrapper::Set_DX8_Texture_Sampler_State(stage, D3DSAMP_MIPFILTER, D3DTEXF_POINT);
	}
