...
Int copyRect(unsigned char *buf, Int bufSize, int oX, int oY, int width, int height)
{
...

	{
		...
		// The destination `buf` is tightly packed (stride = width*bpp, no padding),
		// whereas the locked surface has its own row Pitch (which may include padding),
		// so we copy one row at a time rather than a single block memcpy.
