...
void AIUpdateInterface::privateAttackPosition( const Coord3D *pos, Int maxShotsToFire, CommandSourceType cmdSource )
{
	...
	if (continueRange > 0.0f)
	{
		...
		getObject()->setStatus( MAKE_OBJECT_STATUS_MASK( OBJECT_STATUS_IGNORING_STEALTH ) , true);
		...
		getObject()->setStatus( MAKE_OBJECT_STATUS_MASK( OBJECT_STATUS_IGNORING_STEALTH ) , false);
...
// srj sez: this should ignore calledDuringIdle, despite what the name of the bit implies.
	...
	if ( calledDuringIdle )
	{
		if ((obj->getStatusBits().test( OBJECT_STATUS_STEALTHED )) != 0) 
		{
