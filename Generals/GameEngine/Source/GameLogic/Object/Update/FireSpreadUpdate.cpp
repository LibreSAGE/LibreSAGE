...
/*static*/ void FireSpreadUpdateModuleData::buildFieldParse(MultiIniFieldParse& p) 
{
  UpdateModuleData::buildFieldParse(p);
	...
	{
		{ "OCLEmbers",				ObjectCreationListStore::parseObjectCreationList,		NULL, offsetof( FireSpreadUpdateModuleData, m_oclEmbers ) },
...
UpdateSleepTime FireSpreadUpdate::update( void )
{
	...
	if( (me->getStatusBits().test( OBJECT_STATUS_AFLAME )) == 0 )
...
{
	if ((getObject()->getStatusBits().test( OBJECT_STATUS_AFLAME )) == 0)
