...
void StealthUpdateModuleData::buildFieldParse(MultiIniFieldParse& p) 
{
  UpdateModuleData::buildFieldParse(p);
	...
	static const FieldParse dataFieldParse[] = 
	{
		...
		{ "DisguiseFX",										FXListStore::parseFXList,								NULL, offsetof( StealthUpdateModuleData, m_disguiseFX ) },
		{ "DisguiseRevealFX",							FXListStore::parseFXList,								NULL, offsetof( StealthUpdateModuleData, m_disguiseRevealFX ) },
...
/// @todo srj -- do we need to do this EVERY frame?
	...
	if (allowedToStealth())
	{
		...
		if ( ( self->getStatusBits().test( OBJECT_STATUS_STEALTHED ) ) == 0 )
		{
